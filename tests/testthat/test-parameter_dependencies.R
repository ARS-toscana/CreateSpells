test_that("Error 01 if replace_missing_end_date is missing", {
  ...
  expect_no_error_with_defaults_special_names(dataset = test_data_special_names, replace_missing_end_date = "20100101")
})
...
test_that("Error 02 if replace_missing_end_date is not a date", {
  ...
  test_error_type_special_names(replace_missing_end_date = 123456, error_include = "Error 02")
  test_error_type_special_names(replace_missing_end_date = "123456", error_include = "Error 02")
  test_error_type_special_names(replace_missing_end_date = "abcabc", error_include = "Error 02")
  expect_no_error_with_defaults(replace_missing_end_date = 20100101)
  expect_no_error_with_defaults(replace_missing_end_date = "20100101")
  expect_no_error_with_defaults(replace_missing_end_date = lubridate::ymd("20100101"))
  expect_no_error_with_defaults_special_names(replace_missing_end_date = 20100101)
  expect_no_error_with_defaults_special_names(replace_missing_end_date = "20100101")
  expect_no_error_with_defaults_special_names(replace_missing_end_date = lubridate::ymd("20100101"))
})

test_that("Error 03 if specified column name is not in dataset", {

  test_error_type(id = "a", error_include = "Error 03")
  test_error_type_special_names(id = "a", error_include = "Error 03")
  expect_no_error_with_defaults(id = "person_id")
  expect_no_error_with_defaults_special_names(id = "id")

  test_error_type(start_date = "a", error_include = "Error 03")
  test_error_type_special_names(start_date = "a", error_include = "Error 03")
  expect_no_error_with_defaults(start_date = "op_start_date")
  expect_no_error_with_defaults_special_names(start_date = "start_date")

  test_error_type(end_date = "a", error_include = "Error 03")
  test_error_type_special_names(end_date = "a", error_include = "Error 03")
  expect_no_error_with_defaults(end_date = "op_end_date")
  expect_no_error_with_defaults_special_names(end_date = "end_date")

  test_error_type(category = "a", error_include = "Error 03")
  test_error_type_special_names(category = "a", error_include = "Error 03")
  expect_no_error_with_defaults(category = NULL)
  expect_no_error_with_defaults(category = "op_meaning")
  expect_no_error_with_defaults_special_names(category = NULL)
  expect_no_error_with_defaults_special_names(category = "category")
})
...
test_that("Error 04 if want to calculate overlap without specifying the category argument", {
  ...
  test_error_type_special_names(overlap = T, error_include = "Error 04")
  expect_no_error_with_defaults(overlap = T, category = "op_meaning")
  expect_no_error_with_defaults_special_names(overlap = T, category = "category")

  test_error_type(only_overlaps = T, error_include = "Error 04")
  test_error_type_special_names(only_overlaps = T, error_include = "Error 04")
  expect_no_error_with_defaults(only_overlaps = T, category = "op_meaning")
  expect_no_error_with_defaults_special_names(only_overlaps = T, category = "category")
})
...
test_that("Error 05 if want to calculate overlap with only one category", {
  ...
  test_error_type_special_names(overlap = T, error_include = "Error 05", category = "category",
                  dataset = test_data_special_names[test_data_special_names$category == "a", ])
  expect_no_error_with_defaults(overlap = T, category = "op_meaning")
  expect_no_error_with_defaults_special_names(overlap = T, category = "category")
  ...
  test_error_type_special_names(only_overlaps = T, error_include = "Error 05", category = "category",
                  dataset = test_data_special_names[test_data_special_names$category == "a", ])
  expect_no_error_with_defaults(only_overlaps = T, category = "op_meaning")
  expect_no_error_with_defaults_special_names(only_overlaps = T, category = "category")
})
...
test_that("Error 06 if dataset_overlap has been specified but overlap or only_overlaps are missing", {
  ...
  test_error_type_special_names(dataset_overlap = "df_overlap", error_include = "Error 06", category = "category")
  ...
  expect_no_error_with_defaults_special_names(dataset_overlap = "df_overlap", category = "category", overlap = T)
  expect_no_error_with_defaults_special_names(dataset_overlap = "df_overlap", category = "category", only_overlaps = T)
})
...
test_that("Error 07 some start dates are missing", {
  ...
  test_error_type(dataset = row_wise_dt(~person_id, ~op_start_date,  ~op_end_date, ~op_meaning,
                                        "a",       20100101,      20200101,         "a",
                                        "a",             NA,      20200101,         "a"), error_include = "Error 07")
  test_error_type_special_names(dataset = row_wise_dt(~id, ~start_date,  ~end_date, ~category,
                                        "a",    "20100101", "20200101",       "a",
                                        "a",            NA, "20200101",       "a"), error_include = "Error 07")
  test_error_type_special_names(dataset = row_wise_dt(~id, ~start_date,  ~end_date, ~category,
                                        "a",      20100101,   20200101,       "a",
...
test_that("Error 08 some start dates are not dates or ymd format", {
  ...
  expect_no_error_with_defaults(dataset = tibble::tribble(~person_id, ~op_start_date, ~op_end_date, ~op_meaning,
                                                          "a",         200101,       200101,         "a"))
  expect_no_error_with_defaults_special_names(dataset = tibble::tribble(~id, ~start_date, ~end_date, ~category,
                                                            "a",  "20200101","20200101",       "a"))
  expect_no_error_with_defaults_special_names(dataset = tibble::tribble(~id, ~start_date, ~end_date, ~category,
                                                            "a",    20200101,  20200101,       "a"))
  expect_no_error_with_defaults_special_names(dataset = tibble::tribble(~id, ~start_date, ~end_date, ~category,
...
test_that("Error 01 if specified column name is not in dataset", {
  test_error_type_overlap(id = "a", error_include = "Error 01")
  test_error_type_overlap_special_names(id = "a", error_include = "Error 01")
  expect_no_error_with_defaults_overlap(id = "person_id")
  expect_no_error_with_defaults_overlap_special_names(id = "id")

  test_error_type_overlap(start_date = "a", error_include = "Error 01")
  test_error_type_overlap_special_names(start_date = "a", error_include = "Error 01")
  expect_no_error_with_defaults_overlap(start_date = "op_start_date")
  expect_no_error_with_defaults_overlap_special_names(start_date = "start_date")

  test_error_type_overlap(end_date = "a", error_include = "Error 01")
  test_error_type_overlap_special_names(end_date = "a", error_include = "Error 01")
  expect_no_error_with_defaults_overlap(end_date = "op_end_date")
  expect_no_error_with_defaults_overlap_special_names(end_date = "end_date")

  test_error_type_overlap(category = "a", error_include = "Error 01")
  test_error_type_overlap_special_names(category = "a", error_include = "Error 01")
  # expect_no_error_with_defaults_overlap(category = NULL)
  expect_no_error_with_defaults_overlap(category = "op_meaning")
