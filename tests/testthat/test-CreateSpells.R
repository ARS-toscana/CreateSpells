test_that("single observation no meaning", {
  result <- row_wise_dt(~id ,~num_spell,~entry_spell_category,~exit_spell_category,
                        "a1",         1,           "20100101",          "20200101")
  expect_identical(test_CreateSpells.internal(dataset = test_data_1[to_use_comment == "single"][, meaning := NULL]),
                   result)
  expect_identical(test_CreateSpells.internal(dataset = test_data_1[to_use_comment == "single"]),
                   result)
})

test_that("pass meaning", {
  expect_identical(test_CreateSpells.internal(dataset = test_data_1[to_use_comment == "single"], category = "meaning"),
...
test_that("partial overlap", {
  expect_identical(test_CreateSpells.internal(dataset = test_data_1[to_use_comment == "partial overlap"]),
